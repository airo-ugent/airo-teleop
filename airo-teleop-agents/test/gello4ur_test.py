...
#=============Test configuration==================#
ur5 = URrtde(ip_address="10.42.0.162")
...
if USE_JOINT_SPACE:
    if CONTROL_ROBOT:  # Slowly move to start position
        ...
        ur5.move_to_joint_configuration(action, joint_speed=0.1).wait()
    while True:
        action = teleop_agent.get_action()
        if CONTROL_ROBOT:
            ur5.servo_to_joint_configuration(action, duration=loop_delay).wait()
    ...
    if CONTROL_ROBOT:  # Slowly move to start position
        ...
        ur5.move_to_tcp_pose(ee_pose, joint_speed=0.1).wait()
    while True:
        ee_pose = teleop_agent.get_action()
        if CONTROL_ROBOT:
            ur5.servo_to_tcp_pose(ee_pose, duration=loop_delay).wait()
